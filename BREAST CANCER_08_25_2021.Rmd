---
title: "BREAST CANCER.AUGUST 2021"
authors: "Abbas Jalili", "Sirpreet Padda"
date : "08/31/2021"
  output:
...
Including only ten predictors in the logistic regression model, accuracy dropped by 2%. Thus, we can infer that accuracy is about 3%, correctly predicting an increase in the benign cell with a cut-off value of 0.5.


Logistic regression equation:
$logit(diagnosis) = - 17.23 - 3.85(radius \ mean) + 0.50 (texture\  mean) - 122.53 (compactness \ mean) + 359.12 (concave\  points \ mean) + 270.83 (smoothness \ se) - 669.50 (fractal\  dimension\  se) + 0.40 (radius \ worst) + 0.04 (area\  worst) + 0.12 (symmetry\  worst) + 235.81 (fractal \ dimension \ worst)$.

Interpretation:

* For every one unit change in radius_mean, the log odds of diagnosis of cancer cells to be benign (versus malignant) decreases by 3.85.
* For every one unit change in texture_mean, the log odds of diagnosis of cancer cells to be benign (versus malignant) increases by 0.5.
* For every one unit change in compactness_mean, the log odds of diagnosis of cancer cells to be benign (versus malignant) decreases by 122.53.
* For every one unit change in concave_points_mean, the log odds of diagnosis of cancer cells to be benign (versus malignant) increases by 359.12.
* For every one unit change in smoothness_se, the log odds of diagnosis of cancer cells to be benign (versus malignant) increases by 270.83.
* For every one unit change in fractal_dimension_se, the log odds of diagnosis of cancer cells to be benign (versus malignant) decreases by 669.50.
* For every one unit change in radius_worst, the log odds of diagnosis of cancer cells to be benign (versus malignant) increases by 0.4.
* For every one unit change in area_worst, the log odds of diagnosis of cancer cells to be benign (versus malignant) increases by 0.04.
* For every one unit change in symmetry_worst, the log odds of diagnosis of cancer cells to be benign (versus malignant) increases by 0.12.
* For every one unit change in fractal_dimension_worst, the log odds of diagnosis of cancer cells to be benign (versus malignant) increases by 235.81.
...
error = function(predicted, actual){
  TEST = round(mean(predicted  != actual),2)*100
...
The optimal cut value is 0.693, which is quite close to 1. The area under the curve is 0.997, indicating that the classifier is accurate. In general, the model is better if the ROC curve is close to one. In other words, when the ROC curve is close to one, that indicates a perfect classification.
...
Surprisingly, the QDA predictions are more spectacular and accurate in capturing the true relationship than the logistic regression. Furthermore, the AUC, in this case, is 0.995, which is close to 1. As a result, our classifier can be considered excellent. When a cell is benign, 0.099 is the best cut-off point. It exhibits sensitivity while maintaining the greatest specificity. An argument best indicates the threshold with the highest sum sensitivity + specificity plotted.
...
```
An aforementioned summary table shows the maximum accuracy and kappa after using an optimal value of k = 9. As a result, we chose k = 9 as the optimal value of k in K-NN.
...
We chose the NN model with ten neurons in the hidden layers, which has a 98 percent accuracy rate. The rule that explains why ten neurons in the hidden layer were chosen is mentioned and added to the references. As a result, we preferred to proceed with ten neurons in the hidden layer. The formula we used to decide the number of hidden layers and nodes in a hidden layer retrieved from a listed article by Kazemi, Somayeh. (2017). 
...
As seen in the summary table, SVM has a 96 percent accuracy, which is comparable to QDA, but NN is capable of competing with all other algorithms. The decision tree algorithm will be used in the next step to determine the test error and accuracy.
...
# Use a boxplot() function using a ggplot approach.
# Define the variables called plot1, plot2, plot3, plot4.
 plot1 = ggplot(train, aes(radius_mean,texture_mean, fill = diagnosis))  +
  geom_boxplot() 
   
plot2 = ggplot(train, aes(compactness_mean,concave_points_mean, fill = diagnosis))  +
  geom_boxplot()  

plot3 = ggplot(train, aes(radius_worst,area_worst, fill = diagnosis))  +
 geom_boxplot()  

plot4 = ggplot(train, aes(fractal_dimension_worst,symmetry_worst, fill = diagnosis))  +
  geom_boxplot() 

# Use a grid.arrange() function to arrange them.
grid.arrange(plot1, plot2, plot3, plot4 ,ncol = 2) 
```
The Scatterplot shows that the independent variables have a significant relationship with the dependent variable. Some independent variables have a positive correlation with the target variable, whereas others have a non-linear relationship. We also applied the ggplot approach to illustrate the median and outliers among the first eight variables, with outliers displayed as black points. The third parameter diagnosis is a target variable is used to classify tumor cell types.
